...
if (len(cmd_args) > 1):
    ...
    # animated graph
    if "-a" in cmd_args:
        data, curr_date, start_date = load_data(ecdc_api, cached_date=config["cached_date"], trim_start=False) # ECDC data
        plot_case_pct_anim(data, watchlist, comparison_countries, start_date, fps=config["fps"])
else:
    data, curr_date, start_date = load_data(ecdc_api, cached_date=config["cached_date"], trim_start=True) # ECDC data

    # graph cases as a percentage of population by day since the first reported case
    # for each of our regions - see watchlist.json
    plot_case_pct(data, watchlist, comparison_countries, curr_date)